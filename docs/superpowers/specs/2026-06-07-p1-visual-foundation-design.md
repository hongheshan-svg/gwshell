...
P1 处理方案（二选一，建议 A）：
- **A（推荐，小改）** 把 `uiFont` 接活：在 `useSettingsEffects.ts` 里 `documentElement.style.setProperty('--font-sans', settings.uiFont || <sans 默认>)`，并把两处默认值改为 sans（`-apple-system, system-ui, …`）。让既有设置变得真实可用，用户还能换 UI 字体。
- **B（更省）** P1 不碰 settings，仅改 CSS token 让 chrome 用 sans；`uiFont` 维持死设置，列入后续清理。

> ⚠️ 选 A 需遵守本仓库 [[dual-appsettings-sync]] 约束：默认值在 `settingsStore.ts:90` 与 `SettingsModal.tsx:124` **两处**都要改，外加 i18n。
...
| `useSettingsEffects.ts` + `settingsStore.ts` + `SettingsModal.tsx` | **仅当选 §4.1 方案 A**：接活 `uiFont`→`--font-sans`，两处默认值改 sans + i18n | 小（可选） |
