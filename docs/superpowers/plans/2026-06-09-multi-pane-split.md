...
function loadTs(rel, requireMap = {}) {
  ...
  new Function('exports', 'require', 'module', js)(
    m.exports,
    (s) => (s in requireMap ? requireMap[s] : nodeRequire(s)),
    m,
  );
  return m.exports;
}
...
assert.deepEqual(
  buildSplitPanes(['a', 'b'], 'zzz', 2),
  ['a', 'b'],
  'active not in tabs -> plain order',
);
...
.terminal-pane-cell > .terminal-pane {
  position: absolute;
  inset: 0;
}
.terminal-pane-cell.is-active-pane {
