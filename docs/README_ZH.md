...
| **SSH** | 端口转发 | 本地 (-L)、远程 (-R)、动态 SOCKS5 (-D) 三种转发 |
| **SSH** | 连接延迟 (ping) | 实时 TCP 延迟检测 |
| **SSH** | SSH config 导入 | 从 `~/.ssh/config` 导入主机为资产 |
| **SFTP** | 文件浏览器 | 树形面板浏览远程文件系统 |
| **SFTP** | 上传 / 下载 | 支持文件与文件夹（递归），带传输进度 |
...
| **终端** | 命令面板与命令块 | Ctrl+K 命令面板，Warp 风格命令块导航 |
| **终端** | 命令片段管理 | 保存和回放命令片段 |
| **终端** | 会话日志 | 终端输出按天追加保存到本地日志文件 |
| **监控** | 服务器面板 | 通过 SSH 实时查看 CPU / 内存 / 磁盘 / 网卡 / 进程 |
| **串口** | 串口控制台 | 连接 COM/ttyUSB 设备，配置波特率 |
| **Docker** | Docker 管理 | 本地或经 SSH 列出容器并进入 Shell |
| **界面** | 暗色 / 亮色主题 | 支持系统主题感知和手动切换 |
| **界面** | 国际化（英文、中文） | 完整双语界面 |
| **界面** | 可折叠侧边栏 | 图标导航侧边栏 |
| **界面** | 标签页拖拽 | 拖拽重排终端标签页 |
| **界面** | 系统托盘 | 最小化到托盘，双击恢复 |
| **安全** | 加密存储与保险库 | 凭据落盘加密，可选主密码应用锁 |
| **数据** | SQLite 持久化 | 所有会话与设置本地存储 |
| **数据** | 导入 / 导出 | 资产备份与恢复（导出文件不含密码） |
...
| SSH | [russh](https://crates.io/crates/russh)（纯 Rust 异步 SSH） |
