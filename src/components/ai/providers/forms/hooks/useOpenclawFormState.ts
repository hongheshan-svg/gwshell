...
import type { OpenClawModel, OpenClawProviderConfig } from "../../../lib/types";
...
export function useOpenclawFormState({
  ...
  // Load existing openclaw provider keys for duplicate detection
  const [existingOpenclawKeys, setExistingOpenclawKeys] = useState<string[]>(
    [],
  );

  useEffect(() => {
    providersApi
      .list("openclaw" as AppId)
      .then((providers) => {
        setExistingOpenclawKeys(
          providers.map((p) => p.id).filter((k) => k !== providerId),
        );
      })
      .catch((e) => {
        console.warn("[OpenclawFormState] Failed to load providers:", e);
      });
  }, [providerId]);
