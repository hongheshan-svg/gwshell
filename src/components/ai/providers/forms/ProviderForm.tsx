...
import { mergeProviderMeta } from "../../lib/providerMetaUtils";
...
import { parseOmoOtherFieldsObject } from "../../lib/omo";
import {
...
export function ProviderForm({
  ...
  const presetEntries = useMemo(() => {
    if (appId === "codex") {
      return codexProviderPresets.map<ProviderFormPresetEntry>((preset, index) => ({
        id: `codex-${index}`,
        preset,
      }));
    } else if (appId === "gemini") {
      return geminiProviderPresets.map<ProviderFormPresetEntry>((preset, index) => ({
        id: `gemini-${index}`,
        preset,
      }));
    } else if (appId === "opencode") {
      return opencodeProviderPresets.map<ProviderFormPresetEntry>((preset, index) => ({
        id: `opencode-${index}`,
        preset,
      }));
    } else if (appId === "openclaw") {
      return openclawProviderPresets.map<ProviderFormPresetEntry>((preset, index) => ({
        id: `openclaw-${index}`,
        preset,
      }));
    }
    return providerPresets
      .filter((p) => !p.hidden)
      .map<ProviderFormPresetEntry>((preset, index) => ({
        id: `claude-${index}`,
        preset,
      }));
  }, [appId]);

  // Cast to any[] to satisfy each hook's local PresetEntry type (structurally compatible)
  const presetEntriesAny = presetEntries as any[];
  ...
  // GWShell: no live provider IDs query available; default to empty (no key-lock)
  const opencodeLiveProviderIds: string[] = [];
  const isOpencodeLiveProviderIdsLoading = false;
  ...
  // GWShell: OpenClaw live provider IDs (stub hook)
  const {
  ...
  const handleSubmit = async (values: ProviderFormData) => {
    ...
    if (appId === "opencode" && !isAnyOmoCategory) {
      ...
      if (!opencodeForm.opencodeProviderKey.trim()) {
        toast.error(t("opencode.providerKeyRequired", { defaultValue: "请填写供应商标识" }));
        return;
      }
      if (!keyPattern.test(opencodeForm.opencodeProviderKey)) {
        toast.error(t("opencode.providerKeyInvalid", { defaultValue: "标识格式无效" }));
        return;
      }
      ...
      if (
        ...
        toast.error(t("opencode.providerKeyDuplicate", { defaultValue: "此标识已被使用" }));
        return;
      }
      if (Object.keys(opencodeForm.opencodeModels).length === 0) {
        toast.error(t("opencode.modelsRequired", { defaultValue: "请至少添加一个模型配置" }));
        return;
      }
    }
    ...
    if (appId === "openclaw") {
      ...
      if (!openclawForm.openclawProviderKey.trim()) {
        toast.error(t("openclaw.providerKeyRequired", { defaultValue: "请填写供应商标识" }));
        return;
      }
      if (!keyPattern.test(openclawForm.openclawProviderKey)) {
        toast.error(t("openclaw.providerKeyInvalid", { defaultValue: "标识格式无效" }));
        return;
      }
      if (isProviderKeyLockStateLoading) {
        toast.error(
      ...
      if (
        ...
        toast.error(t("openclaw.providerKeyDuplicate", { defaultValue: "此标识已被使用" }));
        return;
      }
    }

    // Non-official provider validation: endpoint and API Key required
    // cloud_provider (e.g. Bedrock) handles auth via template vars; skip
    // GitHub Copilot uses OAuth; skip API Key requirement
    ...
    if (activePreset) {
      ...
      // OpenClaw: pass preset suggestedDefaults to submit payload
    ...
    if (!isEditMode && draftCustomEndpoints.length > 0) {
      const customEndpointsToSave: Record<
        string,
        import("../../lib/types").CustomEndpoint
      > = draftCustomEndpoints.reduce(
        ...
        {} as Record<string, import("../../lib/types").CustomEndpoint>,
      );
    ...
    // Determine providerType (from preset on new, from existing data on edit)
    const providerType =
  ...
  const groupedPresets = useMemo(() => {
    return presetEntries.reduce<Record<string, ProviderFormPresetEntry[]>>((acc, entry) => {
  ...
  const {
    ...
    category,
    selectedPresetId,
    presetEntries: presetEntriesAny,
  ...
  const handlePresetChange = (value: string) => {
    ...
    if (appId === "codex") {
      ...
      form.reset({
        name: preset.nameKey ? t(preset.nameKey as any, { defaultValue: preset.name }) : preset.name,
    ...
    if (appId === "gemini") {
      ...
      const env = (preset.settingsConfig as Record<string, unknown>)?.env ?? {} as Record<string, unknown>;
      const config = (preset.settingsConfig as Record<string, unknown>)?.config ?? {} as Record<string, unknown>;
  ...
  return (
    <Form {...form}>
      ...
      >
            ...
            appId === "opencode" && !isAnyOmoCategory ? (
              <div className="space-y-2">
                <Label htmlFor="opencode-key">
                  {t("opencode.providerKey", { defaultValue: "供应商标识" })}
                ...
                <Input
                  ...
                  placeholder={t("opencode.providerKeyPlaceholder", { defaultValue: "my-provider" })}
                  disabled={
                ...
                {additiveExistingProviderKeys.includes(
                  ...
                  !isProviderKeyLocked && (
                    <p className="text-xs text-destructive">
                      {t("opencode.providerKeyDuplicate", { defaultValue: "此标识已被使用" })}
                    </p>
                  )}
                {opencodeForm.opencodeProviderKey.trim() !== "" &&
                  !/^[a-z0-9]+(-[a-z0-9]+)*$/.test(
                    ...
                    <p className="text-xs text-destructive">
                      {t("opencode.providerKeyInvalid", { defaultValue: "标识格式无效" })}
                    </p>
                  )}
                {!(
                  additiveExistingProviderKeys.includes(
                  ...
                  (opencodeForm.opencodeProviderKey.trim() === "" ||
                    ...
                    <p className="text-xs text-muted-foreground">
                      {isProviderKeyLocked
                        ? t("opencod.providerKeyLockedHint", {
                            defaultValue:
                              "该供应商已添加到应用配置中，供应商标识不可修改",
                          })
                        : t("opencode.providerKeyHint", { defaultValue: "配置文件中的唯一标识符" })}
                    </p>
                  )}
              </div>
            ) : appId === "openclaw" ? (
              <div className="space-y-2">
                <Label htmlFor="openclaw-key">
                  {t("openclaw.providerKey", { defaultValue: "供应商标识" })}
                  <span className="text-destructive ml-1">*</span>
                </Label>
                <Input
                  id="openclaw-key"
                  ...
                  placeholder={t("openclaw.providerKeyPlaceholder", { defaultValue: "my-provider" })}
                  disabled={
                    isProviderKeyLocked || isProviderKeyLockStateLoading
                  }
                  className={
                ...
                {additiveExistingProviderKeys.includes(
                  ...
                  !isProviderKeyLocked && (
                    <p className="text-xs text-destructive">
                      {t("openclaw.providerKeyDuplicate", { defaultValue: "此标识已被使用" })}
                    </p>
                  )}
                {openclawForm.openclawProviderKey.trim() !== "" &&
                  !/^[a-z0-9]+(-[a-z0-9]+)*$/.test(
                    ...
                    <p className="text-xs text-destructive">
                      {t("openclaw.providerKeyInvalid", { defaultValue: "标识格式无效" })}
                    </p>
                  )}
                {!(
                  additiveExistingProviderKeys.includes(
                  ...
                  (openclawForm.openclawProviderKey.trim() === "" ||
                    ...
                    <p className="text-xs text-muted-foreground">
                      {isProviderKeyLocked
                        ...
                        : t("openclaw.providerKeyHint", { defaultValue: "配置文件中的唯一标识符" })}
                    </p>
                  )}
              </div>
            ) : undefined
        ...
        {/* Config editors: Codex, Claude, Gemini use different editors */}
        {appId === "codex" ? (
          <>
            <CodexConfigEditor
              authValue={codexAuth}
          ...
          <>
            <div className="space-y-2">
              <Label htmlFor="settingsConfig">{t("provider.configJson", { defaultValue: "配置 JSON" })}</Label>
              <JsonEditor
