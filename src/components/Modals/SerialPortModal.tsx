...
export const SerialPortModal: React.FC = () => {
  ...
  const [form, setForm] = useState<SerialForm>({
    ...defaultForm,
    autofill_rows: defaultAutofillRows.map((r) => ({ ...r })),
  });
      ...
      <div className="ssh-modal" style={{ width: 580 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="ssh-modal-tabs" style={{ justifyContent: 'center' }}>
          {serialTabKeys.map((tab) => (
            ...
            >
              {tab === 'standard' ? t('serial_tab_standard') : t('serial_tab_advanced')}
            </button>
          ))}
        </div>
              ...
              <div className="ssh-form-row">
                ...
                <div className="ssh-form-group">
                  ...
                  <input
                    ...
                    placeholder={t('serial_name_placeholder')}
                  />
                ...
                <div className="ssh-form-group">
                  ...
                  >
                    ...
                    {availablePorts.length === 0 &&
                      ['COM1', 'COM2', 'COM3', 'COM4', '/dev/ttyUSB0', '/dev/ttyUSB1'].map((p) => (
                        <option key={p} value={p}>
                          {p}
                        </option>
                      ))}
                  ...
                  {form.autofill_rows.map((row) => (
                    <div key={row.id} className="autofill-row">
                      ...
                      <div className="autofill-controls">
                        <button
                          className={`autofill-ctrl-btn ${row.enabled ? 'ctrl-green' : ''}`}
                          onClick={() => updateRow(row.id, { enabled: !row.enabled })}
                          title={t('serial_rule_enabled')}
                        >
                          &#10003;
                        </button>
                        <span className="autofill-seq">&#9312;</span>
                        <button
                          className={`autofill-ctrl-btn ${row.sequential ? 'ctrl-green' : ''}`}
                          onClick={() => updateRow(row.id, { sequential: !row.sequential })}
                          title={t('serial_rule_sequence')}
                        >
                          &#10003;
                        </button>
                        <button
                          className={`autofill-ctrl-btn ${row.autoEnter ? 'ctrl-blue' : ''}`}
                          onClick={() => updateRow(row.id, { autoEnter: !row.autoEnter })}
                          title={t('serial_rule_auto_enter')}
                        >
                          <CornerDownLeft size={12} />
                        </button>
                        <button
                          className="autofill-ctrl-btn ctrl-red"
                          onClick={() => removeRow(row.id)}
                          title={t('serial_rule_delete')}
                        >
                          <X size={12} />
                        </button>
                      </div>
